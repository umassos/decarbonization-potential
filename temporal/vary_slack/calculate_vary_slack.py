...
for slack in slack_list:
    for g in groupings: 
        ...
        for job in job_length_list: 
            ...
            global_slack0_emissions = slack0_df.mean().mean()
            group_savings = combined_df[members].mean().mean()

            mean_savings = (group_savings/ global_slack0_emissions).mean()
            mean_savings *= 100
...
meanname = f"{main_save_to_dir}/mean.csv"
