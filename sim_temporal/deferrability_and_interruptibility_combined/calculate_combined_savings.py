...
for slack in slack_list:
    ...
    for job in job_length_list:
        abs_non_interrupt_file = f"{main_data_dir}/job_{job}/slack_{slack}/non_interrupt.csv" # reductions -- the difference already accounted
        abs_interrupt_file = f"{main_data_dir}/job_{job}/slack_{slack}/interrupt.csv" # reductions -- the difference already accounted
        abs_slack0_file = f"{main_data_dir}/job_{job}/slack_{slack}/slack0.csv"

        interrupt_df = pd.read_csv(abs_interrupt_file) # reductions already calculated
        non_interrupt_df = pd.read_csv(abs_non_interrupt_file) # reductions already calculated
