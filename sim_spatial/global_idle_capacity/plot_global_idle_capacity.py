...
emission_df = df.mean(numeric_only=True)
...
for i in idle_cap_list:
    y = savings_df.loc[i]
    annon = f"{y}"
    ax.annotate( annon,(int(i)-2,y-6), fontsize=7, rotation=0)
...
ax.set_ylabel(r"Global Carbon Reduction (%)",fontsize=mainlabelsize)
