...
main_save_to_dir = 'data_output/processed_weighted_job'
...
std_list = [i/100 for i in range(0, 100+1, 20)]
    ...
    for row, slack in enumerate(slack_list): 
        ...
        for member in members:
            ...
            for w, job in enumerate(job_length_list): 
                ...
                abs_combined_file = f"{main_data_dir}/job_{job}/slack_{slack}/{member}_interrupt.csv" # reductions -- the difference already accounted
                combined_df = pd.read_csv(abs_combined_file)  

                region_mean = combined_df.mean()
    
                weighted_savings = region_mean * weight_list[w]
        ...
        mean_global = temp_slack_df.mean(axis=1)

        main_slack_df[slack] = mean_global

    file_name = os.path.join(main_save_to_dir, f"{weight_mode}.csv")
    main_slack_df.index.name = 'error'
    main_slack_df.to_csv(file_name)
